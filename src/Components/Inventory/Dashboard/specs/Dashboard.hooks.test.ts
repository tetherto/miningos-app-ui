...
const mockFns = vi.hoisted(() => ({
  listThings: vi.fn(() => ({ data: undefined, isLoading: false })),
  multiTailLog: vi.fn(() => ({ data: undefined, isLoading: false })),
  site: vi.fn(() => ({ data: undefined, isLoading: false })),
  tailLog: vi.fn(() => ({ data: undefined, isLoading: false })),
  headerStats: vi.fn(() => ({ minersAmount: 0, isLoading: false })),
  subtractedTime: vi.fn(() => Date.now() - 60000),
}))

vi.mock('@/app/services/api', () => ({
  useGetListThingsQuery: (...args: unknown[]) => mockFns.listThings(...args),
  useGetMultiTailLogQuery: (...args: unknown[]) => mockFns.multiTailLog(...args),
  useGetSiteQuery: (...args: unknown[]) => mockFns.site(...args),
  useGetTailLogQuery: (...args: unknown[]) => mockFns.tailLog(...args),
}))
...
describe('useDashboardData', () => {
  ...
  it('returns minersTotal from nested array data', () => {
    mockFns.listThings.mockReturnValueOnce({
      data: [[{ id: '1', type: 'miner-s19' }, { id: '2', type: 'miner-s19' }]],
      isLoading: false,
    })
    const { result } = renderHook(() => useDashboardData())
  ...
  it('calculates inventoryClassification from miner stats data', () => {
    mockFns.multiTailLog
      .mockReturnValueOnce({
        data: [[{
          '0': [{ miner_inventory_status_group_cnt_aggr: { installed: 10, unknown: 1 },
  ...
  it('minerDistribution is empty array when stats loading', () => {
    mockFns.multiTailLog.mockReturnValue({ data: undefined, isLoading: true })
    const { result } = renderHook(() => useDashboardData())
    expect(result.current.minerDistribution).toEqual([])
    mockFns.multiTailLog.mockReset().mockReturnValue({ data: undefined, isLoading: false })
  })
  ...
  it('processes containers with miner types for capacity', () => {
    ...
    const { result } = renderHook(() => useDashboardData())
    expect(result.current).toBeDefined()
    mockFns.listThings.mockReset().mockReturnValue({ data: undefined, isLoading: false })
    mockFns.tailLog.mockReset().mockReturnValue({ data: undefined, isLoading: false })
  })
