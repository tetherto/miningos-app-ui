...
describe('useEnergyCostData', () => {
  ...
  it('returns early with defaults when not multi-site mode', () => {
    vi.mocked(useMultiSiteMode).mockReturnValueOnce({
      siteId: undefined,
      selectedSites: [],
      site: {},
      siteList: [],
    ...
    expect(result.current.isLoading).toBe(false)
    expect(result.current.metrics.avgPowerConsumption.value).toBe(0)
    expect(result.current.revenueData).toBeUndefined()
  })

  it('returns early with defaults when no valid date range', () => {
    const { result } = renderHook(() => useEnergyCostData({ dateRange: undefined }))

    expect(result.current.isLoading).toBe(false)
    expect(result.current.metrics.avgAllInCost.value).toBe(0)
  })

  it('handles selectedSites array when not empty', () => {
    vi.mocked(useMultiSiteMode).mockReturnValueOnce({
      siteId: 'site-1',
      selectedSites: ['site-1', 'site-2'],
      site: {},
      siteList: [],
      setSelectedSites: vi.fn(),
      setSelectedSitesManually: vi.fn(),
    ...
    expect(mockBuildRequestParams).toHaveBeenCalledWith(
      expect.objectContaining({ sites: ['site-1', 'site-2'] }),
    )
  })
})
